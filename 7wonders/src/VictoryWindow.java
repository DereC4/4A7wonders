...
//			InputStream x = VictoryWindow.class.getResourceAsStream(fName);
			try {
				Font victoryfont = Font.createFont(Font.TRUETYPE_FONT, new File("fonts\\Minecraftia.ttf"))
						.deriveFont(12f);
			...
			for (Player p : board.getPlayerList()) {
				players.add(p);
				/*
				 * int totalVp = 0; for (String type : p.getVpSources().keySet()) { totalVp +=
				 * p.getVpSources().get(type); } out.println("Player " + p.getIndex() +
				 * "'s VP: " + totalVp);
				 */
			}
